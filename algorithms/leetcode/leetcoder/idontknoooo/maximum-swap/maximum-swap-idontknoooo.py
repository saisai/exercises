...
https://leetcode.com/problems/maximum-swap/solutions/846837/python-3-greedy-math-explanations/
...
class S:
    ...
    def maximumSwap(self, num: int) -> int:
        ...
        for i in range(n-1):                        # find index where s[i] < s[i+1], meaning a chance to flip
            if s[i] < s[i+1]: break                 # if nothing find, return num
        else: return num
        ...
        return int(''.join(s))      # recreate the integer
