...
def test():
    ...
    for d in soup.find_all('div', attrs={'class': 'rq0escxv rj1gh0hx buofh1pr ni8dbmo4 stjgntxs l9j0dhe7'}):
        ...
        if title.count('\n') > 1:
            ...
            q.put({'link': fb_link, 'title': title, 'desc': "<br />".join(title.split("\n")) + fb_link_2 + fblink })
        #q.put({'link': fb_link, 'title': title, 'desc': title.replace('\n', '<br />') + fb_link_2 + fblink })
    json_text = []
    t_desc = []
    t = []
    num = 297
    with open('test.html', 'w', encoding='utf-8') as outfile:
        ...
        while not q.empty():
            ...
            tbl_html = ''
            tbl_html += '<tr><td>{num:03d}</td><td>{num:03d} - {title}</td><td>{desc}</td></tr>'.format(num=num, title=t.get('title'), desc=t.get('desc'))
