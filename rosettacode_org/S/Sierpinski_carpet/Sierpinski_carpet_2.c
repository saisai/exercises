...
/* Clones a title into larget carpet, or blank if center */
void TitleCarpet(Carpet d, int r, int c, Carpet title) {
    ...
    if((r==1) && c==1) {
        for(k=0; k < title->dim; k++) {
            for(m=0; m < title->dim; m++) {
                d->rows[y0+k][x0+m] = ' ';
            }
        }
    } else {
        for(k=0; k<title->dim; k++) {
            for(m=0; m< title->dim; m++) {
                d->rows[y0+k][x0+m] = title->rows[k][m];
            }
        }
    }
}
...
Carpet Sierpinski(int n){
    ...
    for (col=0; col < 3; col++)
    {
        /* 2 row of tiles of copy - third group poins to same data a frist */
        for(row=0; row < 2; row++)
            TitleCarpet(carpet, row, col, subCarpet);
    }
