...
public class FivenumThread {

    private static class FiveNumRunnable implements Runnable {

        double[] value;

        public FiveNumRunnable(double[] value) {
            this.value = value;
        }

        public double median(double[] x, int start, int endInclusive) {
        ...
        public double[] fivenum(double[] x) {
            for(Double d : x) {
                if(d.isNaN())
        ...
        public void run() {
            ...
            System.out.printf("Thread - %s \n %s\n\n",threadName, Arrays.toString(fivenum(this.value)));
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for(double[] d: xl) {
            ...
            // FiveNumRunnable f = new FiveNumRunnable(d);
            Thread t = new Thread(new FiveNumRunnable(d));
            t.start();
            t.join();
        }
